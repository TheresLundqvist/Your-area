...
def all_products(request):
    ...
    if request.GET:
        ...
        """ Logic to search for products and specific descriptions """
        if 'q' in request.GET:
            query = request.GET['q']
            if not query:
...
def add_product(request):
    ...
    if not request.user.is_superuser:
        messages.error(request, 'Sorry, only store owners have access to add products.')  # noqa
...
def edit_product(request, product_id):
    ...
    if not request.user.is_superuser:
        messages.error(request, 'Sorry, only store owners have access to edit products.')  # noqa
        return redirect(reverse('home'))
...
def delete_product(request, product_id):
    ...
    if not request.user.is_superuser:
        messages.error(request, 'Sorry, only store owners have access to delete products.')  # noqa
        return redirect(reverse('home'))
